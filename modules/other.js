...
function trimpcide() {
	if (game.portal.Anticipation.level > 0) {
		...
		if (getPageSetting('ForceAbandon') == false) {
			if (game.global.fighting && game.global.antiStacks < antistacklimit && game.jobs.Amalgamator.owned > 0 && (Math.floor((new Date().getTime() - game.global.lastSoldierSentAt) / 1000)) >= antistacklimit && !game.global.spireActive)
				forceAbandonTrimps();
		}
		else {
